...
"""
    ...
    def checkNetworkStateFinished(self, result, retval,extra_args=None):
        if 'bad address' in result:
            self.session.openWithCallback(self.InstallPackageFailed, MessageBox, _("Your %s %s is not connected to the internet, please check your network settings and try again.") % ("Galaxy Innovations", "ET-7000 Mini"), type=MessageBox.TYPE_INFO, timeout=10, close_on_any_key=True)
...
#            list.append(getConfigListEntry(tab*2 + _("Position PVR State"), config.plugins.MyMetrixLiteOther.showPVRState, _("helptext")))
        ...
        section = _("mini TV")
        list.append(getConfigListEntry(section + tab + sep*(char-len(section)-len(tab)), ))
        list.append(getConfigListEntry(tab + _("Show in Channel selection?"), config.plugins.MyMetrixLiteOther.channelSelectionShowPig, _("Show Pig Window in Channel selection"), "ENABLED"))
        list.append(getConfigListEntry(tab + _("Show in graphical EPG?"), config.plugins.MyMetrixLiteOther.epgGrafShowPig, _("Show Pig Window in graphical EPG")))
#        list.append(getConfigListEntry(tab + _("Show in Movie Center?"), config.movielist.useslim, _("Setting is the same as\n'") + _("Use slim screen") + _("'\nin\n'") + _("Movie List Setup") + "'", "ENABLED"))
        ...
        list.append(getConfigListEntry(section + tab + sep*(char-len(section)-len(tab)), ))
        if not config.plugins.MyMetrixLiteOther.channelSelectionShowPig.value:
        ...
        if config.plugins.MyMetrixLiteOther.channelSelectionShowPig.value == False and int(config.plugins.MyMetrixLiteOther.SkinDesign.value) > 1 and (config.plugins.MyMetrixLiteOther.channelSelectionStyle.value == "CHANNELSELECTION-1" or config.plugins.MyMetrixLiteOther.channelSelectionStyle.value == "CHANNELSELECTION-2"):
