...
"""
    ...
    def __init__(self, session, args = None):
        ...
        self.myname = ConfigText(default = _("My Backup No. %s") % str(self.myset.value), visible_width = 50, fixed_size = False)
        self.mydate = ""
        self.mylastBackup = ""
        self.mylastRestore = ""
        self.file = []
    ...
    def restore(self):
        ...
        for entries in self.file:
            if "set%sname" %set in entries:
                ...
                s+=1
            elif "set%scolor" %set in entries:
                s+=1
            elif "set%sfont" %set in entries:
                s+=1
            elif "set%sother" %set in entries:
                s+=1
            elif "set%sweather" %set in entries:
                s+=1
    ...
    def showHelpWindowQ(self, result):
        if result:
            ...
            if self.runnext == "delete":
                ...
                self["titleText"].setText(_("Restoring Backup-Set %s ...") %self.myset.value)
    ...
    def delete(self, writeFile = True, restore = False):
        ...
        for entries in self.file:
            if "set%sname" %set in entries and not restore:
                pass
            elif "set%sdate" %set in entries and not restore:
                pass
            elif "set%scolor" %set in entries and not restore:
                pass
            elif "set%sfont" %set in entries and not restore:
                pass
            elif "set%sother" %set in entries and not restore:
                pass
            elif "set%sweather" %set in entries and not restore:
                pass
    ...
    def backupQ(self):
        ...
        set = self.myset.value
        for entries in self.file:
            if "set%sname" %str(set) in entries:
    ...
    def backup(self):
        ...
        self.file+=[("set%sname" %set, name)]
        self.file+=[("set%sdate" %set, date)] 
        self.file+=[("set%scolor" %set, config.plugins.MyMetrixLiteColors.getSavedValue())] 
        self.file+=[("set%sfont" %set, config.plugins.MyMetrixLiteFonts.getSavedValue())]  
        self.file+=[("set%sother" %set, config.plugins.MyMetrixLiteOther.getSavedValue())] 
        self.file+=[("set%sweather" %set, config.plugins.MetrixWeather.getSavedValue())]
        self.writeFile()
    ...
    def __changedEntry(self, refresh = False):
        ...
        if cur == "REFRESH" or refresh:
            name = _("My Backup No. %s") % str(self.myset.value)
            date = _("     ") + _("--- empty ---")
            backup = _("-")
            restore = _("-")
